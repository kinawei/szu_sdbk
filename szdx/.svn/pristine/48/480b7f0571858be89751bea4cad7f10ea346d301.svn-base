...
class lpdo extends PDO
{
    ...
    public function __construct($options)
    {
        ...
        $attrs = empty($options['charset']) ? array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES " . $this->charset, PDO::ATTR_PERSISTENT => true) : array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES " . $options['charset'], PDO::ATTR_PERSISTENT => true);
        try
        {
     ...
     * @Return Array |Object;
     */
    public function get_rows($table, $condition = array(), $getRes = false, $column = array(), $limit = '', $ope = '=', $log = 'AND')
    {
        $fields = $this->get_fields($column);
        $cdt = $this->get_condition($condition, $ope, $log);
        if (!empty($condition)) {
            $cdts = array();
        ...
        $where = empty($condition) ? '' : ' where ' . $cdt;
        $this->sql = 'select ' . $fields . ' from ' . $table . $where . ' ' . $limit;
        // print_r($cdts);
        try
        {
            $this->sql .= $this->tail;
        ...
        $rs = $stmt->fetchAll(parent::FETCH_ASSOC);
        return $rs;
    }
